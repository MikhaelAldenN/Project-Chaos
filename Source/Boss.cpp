...
Boss::Boss()
{
    ...
    // 1. Init Monitor (CRT)
    partMonitor.model = std::make_shared<Model>(device, "Data/Model/Character/TEST_mdl_CRTMonitor.glb");

    // Default Transform Monitor
    partMonitor.position = { 0.0f, 0.6f, 6.5f };
    partMonitor.rotation = { 65.0f, 0.0f, 0.0f };
    partMonitor.scale = { 10.0f, 10.0f, 10.0f };
    partMonitor.floatAxis = { 0.0f, 1.0f, 0.0f }; // Naik Turun

    // 2. Init CPU
    partCPU.model = std::make_shared<Model>(device, "Data/Model/Character/TEST_mdl_CPU.glb");

    // Default Transform CPU (Taruh agak di bawah/belakang monitor atau di samping)
