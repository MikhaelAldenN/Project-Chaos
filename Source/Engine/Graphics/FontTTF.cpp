...
// Struktur Vertex disamakan persis dengan Sprite::Vertex agar bisa menggunakan Shader bawaan engine-mu
struct FontVertex {
...
{
    // 1. Baca file .ttf ke dalam binary buffer
    ...
    // 2. Siapkan list karakter Jepang (ASCII + Hiragana + Katakana + Kanji Kustom)
    std::vector<uint32_t> codepoints = GenerateDefaultGlyphList(customCodepoints);

    // 3. Alokasikan memori untuk paking tekstur sementara (Monochrome Alpha)
    ...
    // Mengatur agar perataan piksel font tajam (khusus untuk pixel font seperti zpix)
    ...
    // 5. Konversi tekstur 8-bit Alpha menjadi 32-bit RGBA agar bisa diproses Shader standar
    ...
    for (size_t i = 0; i < rgbaPixels.size(); ++i) {
        ...
        rgbaPixels[i] = (alpha << 24) | (0xFF << 16) | (0xFF << 8) | 0xFF; // Putih dengan alpha dinamis
    }

    // 6. Unggah Tekstur ke DirectX 11 VRAM GPU
...
std::vector<uint32_t> FontTTF::GenerateDefaultGlyphList(const std::vector<uint32_t>& customKanji)
{
    ...
    // Basic ASCII (Karakter Keyboard standar)
    for (uint32_t cp = 32; cp <= 126; ++cp) list.push_back(cp);

    // =========================================================
    // [NEW] CJK Symbols & Punctuation (Termasuk 〜, 。, 、, 「」)
    // =========================================================
    ...
    // Simbol & Tanda baca Jepang (Full-width)
    for (uint32_t cp = 0xFF00; cp <= 0xFFEF; ++cp) list.push_back(cp);

    // Masukkan Kanji kustom yang dikirim lewat parameter
...
{
    // Algoritma konversi Byte UTF-8 string menjadi Unicode Codepoint murni (Tanpa Shortcut)
...
{
    // Gunakan spesifikasi Layout yang sama persis dengan Sprite.cpp milikmu
    ...
    // Alokasikan Buffer Vertex Dinamis (Maksimal menampung string berisi 1000 huruf sekaligus per draw call)
...
void FontTTF::Draw(const std::string& utf8Text, float startX, float startY, float scale, DirectX::XMFLOAT4 color)
{
    ...
    while (charIdx < utf8Text.size()) {
        ...
        float ndcY0 = 1.0f - (2.0f * y0 / screenH); // Y dibalik karena koordinat DirectX Y+ ke atas
        float ndcX1 = (2.0f * x1 / screenW) - 1.0f;
        float ndcY1 = 1.0f - (2.0f * y1 / screenH);

        // Bentuk urutan Triangle List (6 Titik) dengan Koordinat NDC yang sudah dikonversi
    ...
    // Kirim data koordinat teks ke GPU Context
    auto dc = Graphics::Instance().GetDeviceContext();
    ...
    // Set Sistem Viewport Render Matrix dari NDC layar (Adaptasi logika math dari Sprite.cpp milikmu)
